...
/**
 * Отправка батча с подписью SMS кодом и получение информации по обработке отправленных записей
 *
...
$batchRecords =  <<<'BATCH'
WM;R555511198765;51.21;RUR;1009;Payout 9
...
$pushBatchResult = $client->pushBatchAdvanced(
    ...
    $accountUSD,
    $accountBTC,
    \capitalist\api\VerificationTypesInterface::SMS_CODE_MOBILE,
    null
);
...
/** После получения SMS кода, подписываем платеж */

/**
 * Подпись массового платежа в систему (операция process_batch)
 */
$processBatchResult = $client->processBatch(
    $pushBatchResultArray['data']['id'],
    \capitalist\api\VerificationTypesInterface::SMS_CODE_MOBILE,
    $sampleSmsCode
);
